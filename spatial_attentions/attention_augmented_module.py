...
class AugmentedConv(nn.Module):
    ...
    def forward(self, x):
        ...
        return torch.concat((conv_out, attn_out), dim=1)
    ...
    def relative_logits(self, q):
        ...
        key_rel_w = torch.randn((2 * W - 1, dk))
        rel_logits_w = self.relative_logits_1d(q, key_rel_w, H, W, Nh, "w")

        key_rel_h = torch.randn((2 * H - 1, dk))
        rel_logits_h = self.relative_logits_1d(
            torch.transpose(q, 2, 3), key_rel_h, W, H, Nh, "h")

        return rel_logits_h, rel_logits_w

    def relative_logits_1d(self, q, rel_k, H, W, Nh, case):
        rel_logits = torch.matmul(q, rel_k.transpose(0, 1))
